...
Serve.prototype.createContainerForGitRepository = function (repository) {
  return new Promise(function (resolve, reject) {
    ...
    var commands = [
      'bash',
      '-c',
      'git clone ' + repository + ' app && cd app && swift build --configuration release && PATH=$PWD/.build/release:$PATH PORT=8000 foreman start web'
    ];
    ...
    return this.docker.createContainerAsync({
      Image: process.env.CONTAINER_ID,
      Cmd: commands,
      Detach: true,
      HostConfig: {
        Privileged: true,
...
var createNginxConfig = function createNginxConfig (name, port, callback) {
  return new Promise(function (resolve, reject) {
    ...
    var useSsl = (fs.statSync(paths.cert).isFile() && fs.statSync(paths.key).isFile());
    var template = 'server {\n' +
      'listen 80;\n' +
      templateMain(false) +
    '}';
