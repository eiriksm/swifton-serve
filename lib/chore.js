  ...
  return new CronJob(interval + ' * * * * *', function () {
    ...
    .then(function (entries) {
      ...
      return new Promise(function (resolve, reject) {
            ...
            if (container) {
              if (entry.status !== 'running') {
                ...
                .then(function (stopped) {
                  if (!stopped) {
                    console.log('Chore: Docker Checkup for', key, 'owned by someone else. Better not touching this…');
                    return resolve();
                  }
                ...
                .catch(function (err) {
                  console.error('Chore:', 'Docker Checkup for', key, '[ERROR] ->', err);
                  return resolve();
                });
              } else {
        ...
        // emergency resolve so we don't fall through
        resolve();
      }.bind(this));
    }.bind(this))
    ...
    .then(function () {
      return new Promise(function (resolve, reject) {
        ...
        for (var key in this.theContainers) {
          if (this.theContainers.hasOwnProperty(key)) {
            ...
            if (!entry) {
              ...
              .then(function (stopped) {
                if (!stopped) {
                  console.log('Chore: CouchDB Checkup for', key, 'owned by someone else. Better not touching this…');
                  return resolve();
                }
                var confPath = path.join(__dirname, '../', 'vhosts', container.id + '.conf');
                console.log('Chore:', 'Removing Vhost @', confPath);
                ...
                return resolve(leftEntries);
              })
              .catch(function (err) {
                console.error('Chore:', 'CouchDB Checkup for', key, '[ERROR] ->', err);
                return resolve(leftEntries);
              });
            } else {
        ...
        // emergency resolve to not end up here…
        resolve(leftEntries);
      }.bind(this));
    }.bind(this))
